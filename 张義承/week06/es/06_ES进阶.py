...
def print_search_results(response):
    ...
    for hit in response["hits"]["hits"]:
        print(
            f"得分：{hit['_score']}，文档内容：{json.dumps(hit['_source'], ensure_ascii=False, indent=2)}"
        )
...
es = Elasticsearch(
    ELASTICSEARCH_URL,
    basic_auth=("elastic", "123456"),  # 用户名, 密码
    verify_certs=False,  # HTTPS 自签名证书，开发环境关闭校验
    request_timeout=30,
)
...
res_1 = es.search(
    index=index_name,
    body={
        "query": {"multi_match": {"query": "智能", "fields": ["name", "description"]}}
    },
)
...
res_3 = es.search(
    index=index_name,
    body={
        "aggs": {"products_by_category": {"terms": {"field": "category", "size": 10}}},
